...
├── data.py       ← Dataset, augmentation, train/val/test split, class weights
...
### 2.4 Stratified train/val/test bölme (`data.py::get_train_val_test_split`)

Doküman §3.2'ye uygun olarak **stratified 70/15/15 train/val/test split** kullanılır. İki aşamalı `StratifiedShuffleSplit`:

1. Önce `test_frac=0.15` ile test seti tüm veriden held-out (stratified, seed=42).
2. Kalan %85'in içinden `val_rel = 0.15 / 0.85 ≈ 0.1765` ile val seti ayrılır.

**Sabit dağılım (170 örnek, seed=42):**
...
| 4AA | 43 | 9 | 9 | 61 |
| Cleavage | 21 | 4 | 4 | 29 |
| **Toplam** | **120** | **25** | **25** | **170** |

**Önemli kısıt:** Cleavage sınıfı için test paylaşımı **4 örnek** — sınıf-spesifik recall metriğinin paydası 4 olduğundan, **1 yanlış tahmin yaklaşık ±%25 oynama** yaratır. Bu yapısal bir kısıttır; final yorumda weighted P/R/F1 ve confusion matrix birlikte değerlendirilir.

**Sızıntı (leakage) garantisi:**
- Test seti **eğitim, hiperparametre seçimi ve early stopping** süresince hiç kullanılmaz.
- Class weights yalnızca `train_idx` üzerinden hesaplanır.
- Early stopping sinyali sadece val seti `val_loss` üzerinde çalışır.
...
### 4.1 İki aşamalı eğitim (`train.py::train_single_run`)
...
Validasyon kaybı 10 epoch art arda iyileşmezse eğitim durur (doküman §3.2 talebi). En iyi val_loss veren epoch'taki ağırlıklar `best_model.pth` olarak kaydedilir — son epoch değil!
...
### 5.1 Test seti üzerinde tek-koşum değerlendirmesi

`evaluate.py::evaluate_test`:
1. `train_result["checkpoint_path"]` (= `best_model.pth`) yüklenir
2. Held-out test indeksleri üzerinde `predict_loader` çalıştırılır
3. Weighted P/R/F1, accuracy, classification report, confusion matrix, ROC AUC (one-vs-rest) hesaplanır
4. `outputs/predictions_test.csv` her test görüntüsü için tahmin kaydı tutar — morphology raporu bunu okur

### 5.2 Üretilen artefaktlar

| Çıktı | Dosya | Doküman §'a karşılık |
|---|---|---|
| Train/val acc-loss grafiği | `figures/history.png` | §4.1 |
| Test confusion matrix (raw) | `figures/confusion_matrix_test.png` | §4.2 |
| Test confusion matrix (normalize) | `figures/confusion_matrix_test_norm.png` | §4.2 |
| ROC eğrileri (one-vs-rest) | `figures/roc_curves.png` | (bonus) |
| Learning curve | `figures/learning_curve.png` | §4.1 |
| Test metrikleri ve split bilgisi | `reports/metrics_summary.json` | §4.2 |
| Test tahmin tablosu | `outputs/predictions_test.csv` | §5.3 girdi |
...
Tek koşumda doğrudan en düşük `val_loss` veren epoch'un ağırlıkları `best_model.pth` olarak kaydedilir. `export_final_model()` argümansız çağrılır ve bu dosyayı `final_model.pth` adıyla kopyalar — Flask app ve inference scriptleri kanonik adı bekler.
...
| | TRAIN/VAL/TEST_FRAC | 0.70 / 0.15 / 0.15 | Doküman §3.2 |
...
| Validasyon: stratified 70/15/15 | 5-fold CV | Doküman §3.2 literal uyumu; held-out test seti üzerinde tek-koşum raporu |
...
4. **Sağlam değerlendirme** — held-out test set + weighted P/R/F1 + ROC AUC per class + learning curve
5. **Klinik kullanılabilirlik** — Grad-CAM + güven uyarısı + morfolojik analiz + geçmiş takip

Sonuç: held-out test setinde weighted P/R/F1 raporlanan, sağlam güven kalibrasyonlu, açıklanabilir karar süreci sunan bir sistem.
